...
yVec = zeros(m,num_labels);  %num_labels is the number of classes in the output

for i = 1:m                 %this is basically creating the 00100.. vector
    yVec(i,y(i)) = 1;  %yVec would contain the whole matric of 0s and 1.
end
...
for t = 1:m
	...
	a1 = [1; X(t,:)'];      %adding 1(bias unit) to the column vector
	...
	yy = ([1:num_labels]==y(t))'; %yy is the h(x)
	% For the delta values:
	delta_3 = a3 - yy;

	delta_2 = (Theta2' * delta_3) .* [1; sigmoidGradient(z2)];% ((26*10)*(10*1))=(26*1)
	delta_2 = delta_2(2:end); % Taking of the bias row

	% delta_1 is not calculated because we do not associate error with the input    

	% Big delta update
	Theta1_grad = Theta1_grad + delta_2 * a1'; % (10*1)*(1*26)
	Theta2_grad = Theta2_grad + delta_3 * a2';  % (25*1)*(1*401)
end

%UNREGULARIZED FORM
Theta1_grad = (1/m) * Theta1_grad;%(10*26)
Theta2_grad = (1/m) * Theta2_grad;%(25*401)
